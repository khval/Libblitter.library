...
void CustomToBlitsate( struct Custom *custom, struct blitstate *bs )
{
	...
	blitzenWrite( BLTALWM, custom -> bltalwm, &handled, bs );
	blitzenWrite( BLTCPTH, (uint32) custom -> bltcpt >> 16, &handled, bs );
	blitzenWrite( BLTCPTL, (uint32) custom -> bltcpt, &handled, bs );
	blitzenWrite( BLTBPTH, (uint32)custom -> bltbpt >>16, &handled, bs );
	blitzenWrite( BLTBPTL, (uint32) custom -> bltbpt, &handled, bs );
	blitzenWrite( BLTAPTH, (uint32) custom -> bltapt >>16, &handled, bs );
	...
	blitzenWrite( BLTDMOD, custom -> bltdmod, &handled, bs );
